...
def clusterize(locations, database, totaln_filename, HOMEP_filename, seqid_thr, tmscore_thr):
	obj_listofsets = []
	fam_listofsets = []
	...
	for line in text:
		if not line:
			continue
		fields = line.split()
		if not (fields[0], fields[1]) in split_text:
			split_text[(fields[0], fields[1])] = []
		split_text[(fields[0], fields[1])].append(line)


	structsets = []
	for superfamily in sorted(list(split_text.keys())):
		structset = set()
		for line in split_text[superfamily]:
			fields = line.split()
			structset.add(fields[2])
		structsets.append(frozenset([x for x in structset]))
	...
	HOMEP_library = {}
	for superfamily in sorted(list(split_text.keys())):
		superfamily_name = superfamily[0] + str(int(superfamily[1]))	
		...
		for line in split_text[superfamily]:
			...
			if float(fields[5]) > tmscore_thr:
				fam_listofsets.append(frozenset([fields[2], fields[3]]))
				if float(fields[4]) > seqid_thr:
					obj_listofsets.append(frozenset([fields[2], fields[3]]))
